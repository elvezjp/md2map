...
3 つの分割モードを提供する。`heading` モードはデフォルトで追加依存なし。`nlp`・`ai` モードは見出しベース分割後に、閾値を超えるセクションの自身コンテンツ範囲を再分割し「仮想見出し」を挿入する。
...
4. 分割モードに応じた境界決定を実行
5. 仮想セクションを元セクションの直下（子セクションの前）に挿入し、階層関係を再構築
...
対象セクションの自身コンテンツ範囲のテキストに行番号を付与して LLM に送信する。行番号の形式は [add-line-numbers](https://github.com/elvezjp/add-line-numbers) に準拠する（`{行番号:4d}: {行内容}`、右揃え 4 桁 + コロン + スペース）。
...
| 注意事項 | 行の連続性・網羅性・非重複の制約、タイトルの言語指定 |
...
**フィールド検証**:
- `start_line`: 自身コンテンツ範囲の開始行以上の整数であること
- `end_line`: `start_line` 以上かつ自身コンテンツ範囲の終了行以下の整数であること
- `title`: 文字列（空文字列の場合はフォールバックタイトルを使用）

**構造検証**（すべて満たさない場合はフォールバック分割）:
- 最初のグループの `start_line` が自身コンテンツ範囲の開始行と一致すること
- 最後のグループの `end_line` が自身コンテンツ範囲の終了行と一致すること
