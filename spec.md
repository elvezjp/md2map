...
- **仮想見出し**: NLP/AI モードで生成されるサブセクションは「仮想見出し」として扱い、元文書の見出し構造を変更せずに分割粒度を細分化する
...
3 つの分割モードを提供する。`heading` モードはデフォルトで追加依存なし。`nlp`・`ai` モードは見出しベース分割後に、閾値を超える末端セクションを段落単位で再分割し「仮想見出し」を挿入する。

| モード | 概要 | 再分割の境界決定方法 |
|--------|------|---------------------|
| `heading` | 見出し階層のみで分割 | なし（再分割しない） |
| `nlp` | 形態素解析で意味的境界を検出 | 隣接段落間の名詞 Jaccard 類似度が低い箇所 |
| `ai` | LLM に段落分割とタイトル生成を委任 | LLM が返す段落グループとタイトル |

**再分割の条件**:
- セクションの文字数（日本語）/単語数（英語）が `--split-threshold` 以上
- セクションが子見出しを持たない（末端セクション）
- セクション内に 2 つ以上の段落が存在する

**仮想見出しの生成**:
- 仮想セクションは元セクションのレベル + 1（最大 6）で生成される
- NLP モードでは `<元セクション名> (part N/M)` 形式のタイトルが付与される
- AI モードでは LLM が生成した意味的タイトルが付与される（LLM 応答が不正な場合は閾値ベースのフォールバック分割を行う）
...
### 3.3 セクション再分割フェーズ（NLP/AI モード）

`--split-mode` が `nlp` または `ai` の場合、見出しベースで抽出されたセクションのうち、再分割条件を満たすものに対して仮想見出しを挿入する。

**再分割の全体手順**:

1. 各セクションについて再分割条件（末端セクションかつ閾値超過かつ段落数 2 以上）を判定
2. 対象セクションの本文を段落単位に分割（空行を境界とする）
3. 目標分割数を算出: `min(max_subsections, max(2, 文字数 / 閾値))` かつ段落数以下
4. 分割モードに応じた境界決定を実行
5. 仮想セクションを元セクションの直下に挿入し、階層関係を再構築
...
**AI モードの境界決定**:
- 段落に番号を付与して LLM に送信
- LLM は段落グループとタイトルを JSON 配列で返却
- レスポンスの妥当性を検証（全段落のカバー・連続性・非重複）
- LLM 応答が不正または API エラーの場合は閾値ベースの均等分割にフォールバック

**閾値ベースの均等分割**（フォールバック）:
- 目標分割数に基づき段落あたりの目標文字数を算出
- 段落を順に積み上げ、目標文字数を超えた時点でチャンクを切り出す
...
NLP モードでは LLM を使用しないため、仮想見出しのタイトルは以下の形式で自動生成する：
...
対象セクションの本文を空行区切りで段落に分割し、各段落に `[N]` 形式の連番を付与して LLM に送信する。
...
| 目的 | 連番付き段落群を意味的なまとまりごとにグループ化し、各グループにタイトルを付与する |
| 出力形式 | JSON 配列のみを返却。各要素は `title` (string)、`start_paragraph` (integer)、`end_paragraph` (integer) のフィールドを持つ |
| 注意事項 | 段落の連続性・網羅性・非重複の制約、タイトルの言語指定 |

**ユーザーメッセージ**:
- 最大分割数の指示（「以下の段落群を最大 N つのセクションに分割してください。」）
- 段落一覧（`[1] <本文>`, `[2] <本文>`, ... の形式）

### 6.3 LLM からの出力と検証

LLM のレスポンスを JSON としてパースし、以下の検証を行う。

**前処理**:
- レスポンスが ` ```json ... ``` ` で囲まれている場合、コードブロックを除去する

**フィールド検証**:
- `start_paragraph`: 1 以上の整数であること
- `end_paragraph`: `start_paragraph` 以上の整数であること
- `title`: 文字列（空文字列の場合はフォールバックタイトルを使用）

**構造検証**（すべて満たさない場合はフォールバック分割）:
- 最初のグループの `start_paragraph` が 1 であること
- 最後のグループの `end_paragraph` が総段落数と一致すること
- 前のグループの `end_paragraph + 1` が次のグループの `start_paragraph` と一致すること（隙間なし・重複なし・順序通り）

### 6.4 フォールバック

以下のいずれかに該当する場合、閾値ベースの均等分割にフォールバックする：

- LLM の API 呼び出しが例外で失敗した場合
- レスポンスが JSON としてパースできない場合
- 構造検証に失敗した場合（隙間・重複・カバレッジ不足）

フォールバック時は NLP モードと同様の `<元セクション名> (part N/M)` 形式のタイトルが付与される。

### 6.5 仮想見出しの命名

- LLM が有効なタイトルを返した場合: そのタイトルをそのまま使用する
- LLM のタイトルが空文字列の場合: `<元セクション名> (part N/M)` 形式のフォールバックタイトルを使用する
