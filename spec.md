...
見出し一覧を軽量に取得するコマンド。分割実行前にセクション構造を確認するために使用する。LLM 不要で高速に動作する。

| 引数/オプション | 必須 | デフォルト | 説明 |
|----------------|------|-----------|------|
| `input_file` | 必須 | - | 解析対象のマークダウンファイルパス |
| `--max-depth <N>` | 任意 | 6 | 取得対象の最大見出し深さ（1〜6） |

出力は JSON 形式で標準出力に出力する。各要素は以下のフィールドを持つ：

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `title` | string | 見出しテキスト |
| `level` | integer | 見出しレベル（1-6） |
| `start_line` | integer | セクション開始行番号（1-based） |
| `end_line` | integer | セクション終了行番号（1-based, inclusive） |
| `estimated_chars` | integer | セクション内の文字数（見出し行を含む） |

`start_line` は `build` コマンドのセクション分割結果と同一の値を返すため、`--section-overrides` のキーとして安全に使用できる。
...
| `ai` | LLM に行番号付きテキストを送信し分割位置の決定を委任 | LLM が返す行範囲グループ |
...
- NLP モード・AI モードともに `<元セクション名>: part-N` 形式のタイトルが付与される
...
build コマンドと同じ `_extract_headings()` + `_build_sections()` を使用するため、`start_line` / `end_line` が build の結果と一致することを保証する。サブスプリットやファイル生成は行わない。
...
各セクションについて、`start_line` をキーとしてオーバーライドマップを参照し、適用する設定を決定する。

```
設定の優先順位: セクション単位の override > コンストラクタ引数（デフォルト設定）
```

**LLM provider / NLP tokenizer の遅延初期化**:

デフォルトの `split_mode` が `heading` であっても、オーバーライドにより特定セクションで `ai` や `nlp` モードが必要になる場合がある。この場合、LLM provider や NLP tokenizer はセクション処理時に遅延初期化される。コンストラクタ時点で既に初期化済みの場合は何もしない（冪等）。

**再分割の全体手順**:

1. 各セクションについてオーバーライド設定を解決する（`_resolve_settings()`）
2. 各セクションについて自身コンテンツ範囲を算出する
   - 末端セクション: 見出し行の次〜セクション終了行
   - 子セクションを持つ場合: 見出し行の次〜最初の子セクション開始行の前行
3. 解決された `split_mode` が `heading` の場合はスキップ
4. 自身コンテンツ範囲の文字数が解決された閾値以上かを判定
5. 目標分割数を算出: `min(max_subsections, max(2, 文字数 / 閾値))`
6. 分割モードに応じた境界決定を実行（必要に応じて LLM provider / NLP tokenizer を遅延初期化）
7. サブスプリットセクションを元セクションの直下（子セクションの前）に挿入し、階層関係を再構築
...
システムプロンプトは 4 パート構成の辞書 `DEFAULT_AI_PROMPT_PARTS` で定義される。`role`・`purpose`・`format` の 3 パートはレスポンスバリデーションや機能の根幹と密結合しているため外部変更不可。`notes` パートのみ `ai_prompt_extra_notes` による追記が可能。

| パートキー | 見出し | 内容 | 外部変更 |
|-----------|--------|------|----------|
| `role` | `# 役割` | 文書構造の分析に特化したアシスタント | 不可 |
| `purpose` | `# 目的` | 行番号付きテキストを、文書の構造や意味的なまとまりを保ちつつ話題や内容の切れ目で区切る | 不可 |
| `format` | `# 出力形式` | JSON 配列のみを返却。各要素は `start_line` (integer)、`end_line` (integer) のフィールドを持つ | 不可 |
| `notes` | `# 注意事項` | 行の連続性・網羅性・非重複の制約、ネスト構造保護、分割バランス | 追記可 |

**`notes` パートへの追記**:

`MarkdownParser` の `ai_prompt_extra_notes` 引数（`Optional[str]`）で指定したテキストが、デフォルトの注意事項末尾に改行区切りで追記される。CLI では `--ai-prompt-extra-notes` オプションで指定する。
...
parser = MarkdownParser(
    ...
    ai_prompt_extra_notes="- Mermaid ブロックの途中では分割しないこと",
)
```

**ユーザーメッセージ**:
- 最大分割数の指示と総行数の制約（「以下のテキストを、意味的なまとまりを保ちつつ最大 N つに区切ってください。テキストは全 M 行です。最後の区間は行 M で終了してください。」）
- 1 始まりの行番号付きテキスト全文
...
- LLM の API 呼び出しが例外で失敗した場合（`warnings` にエラーメッセージを追加）
...
NLP モードと同様に `<元セクション名>: part-N` 形式のタイトルを使用する。LLM にはタイトル生成を求めず、分割位置の決定に集中させる。
...
**エラーハンドリング**:
- LLM の API 呼び出しが例外で失敗した場合、`warnings` にエラーメッセージを追加し、サマリーは `None`（未生成）とする
