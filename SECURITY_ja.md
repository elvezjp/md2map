...
本リポジトリでは以下の方針で Dependabot アラートを運用します。

**Malware タブ**: 発生場所を問わず必ず修正対応する

**Vulnerable**: **修正対応**（依存更新／PR作成）する

**補足**: v0.4.3 まではリポジトリ内に旧バージョンのスナップショット（`versions/`）と依存ライブラリの subtree（`add-line-numbers/`）を保持しており、それらの manifest / lockfile に対しても Dependabot アラートが発生していました。両ディレクトリは 0.5.0 で削除済みのため（バージョン管理は git tag に移行、README 参照）、以降のアラートはリポジトリルートの manifest に対してのみ発生します。削除前のディレクトリ由来のアラートが残っている場合は、影響を確認のうえ Dismiss して close します。
