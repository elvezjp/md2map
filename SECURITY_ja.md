...
**補足**: v0.4.3 まではリポジトリ内の `versions/` 配下に旧バージョンのスナップショットを保持しており、それらの lockfile に対しても Dependabot アラートが発生していました。`versions/` は 0.5.0 で削除済みのため（バージョン管理は git tag に移行、README 参照）、以降のアラートはリポジトリルートの manifest に対してのみ発生します。削除前の旧バージョン由来のアラートが残っている場合は、影響を確認のうえ Dismiss して close します。
