...
- **`uv.lock` の解決済みコミットを更新**: 上流で `v0.1.3` タグが貼り直されたため、`uv.lock` が記録する `add-line-numbers` のコミットを `f81b097` から `9b6568c` に更新しました
...
- **`docs/examples/v0.5.1/` を追加**: 本リリースの実装で各コマンドを実行した出力例を追加しました
  - heading / nlp モードと `headings` コマンドの出力は `v0.5.0` と完全に一致します（実装変更がないため）
  - AI モード（`output-ai` / `output-ai-summary`）は LLM の応答に依存するため、`v0.5.0` とは要約文などに差分があります
